...
class Storage:
    ...
    max_operations: cython.uint
    _operations: cython.uint
    ...
    async def __aenter__(self, *args, step:cython.bint=0, **kwargs):
        ...
        if(not os.path.exists(self.storage_base_path)):
            os.mkdir(self.storage_base_path)
        elif(not os.path.isdir(self.storage_base_path)):
    ...
    def add_data(self, key:str, reg:tuple) -> None:
    ...
    def load_data(self, domain, file, *, check_exists: cython.bint=False) -> list:
        path = f"{self.storage_path}/{domain}/{file}.json"
        if(check_exists and not (os.path.exists(path) and os.path.isfile(path))):
            return []
    ...
    def fill_file_cache(self) -> None:
        if(self.storage_path is not None and os.path.exists(self.storage_path) and os.path.isdir(self.storage_path)):
