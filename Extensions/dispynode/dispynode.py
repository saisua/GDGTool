...
def dispy_provisional_result(result, relay=False, timeout=MsgTimeout):
    ...
    try:
        if relay:
            sock.connect(__dispy_job_reply_)
        else:
...
class _DispyNode(object):
    ...
    def __init__(self, cpus, hosts=[], ext_hosts=[], name='', scheduler_host=None,
                 ipv4_udp_multicast=False, dest_path_prefix='', clean=True, node_globals={},
    ...
    def tcp_server(self, addrinfo, task=None):
        ...
        dispynode_logger.info('TCP server at %s:%s', addrinfo.ip, self.port)
        tcp_sock.listen(30)
...
def run(*args, **kwargs):
    ...
    cfg = kwargs.pop('save_config', None)
    if cfg:
        import configparser
        for key, val in list(kwargs.items()):
            if not isinstance(val, str):
                kwargs[key] = str(val)
        kwargs = configparser.ConfigParser(kwargs)
        cfg = open(cfg, 'w')
        kwargs.write(cfg)
        cfg.close()
        exit(0)

    if kwargs.pop('loglevel', None):
    ...
    if(not "hosts" in kwargs):
        print("Warning: Per testing, the default ip has been set to localhost")
        kwargs["hosts"] = ["127.0.0.1"]
    dispy.config.DispyPort = kwargs.pop('dispy_port', 9700)
    cpus = multiprocessing.cpu_count()
    if kwargs.get('cpus'):
        if kwargs.get('cpus') > 0:
            if kwargs.get('cpus') > cpus:
    ...
    if kwargs.get('zombie_interval'):
        if kwargs.get('zombie_interval') < 1:
            raise Exception('zombie_interval must be at least 1')

    if kwargs.get('ping_interval'):
        if kwargs.get('ping_interval') < 10:
            raise Exception('ping_interval must be at least 10')
    else:
        kwargs['ping_interval'] = 5000

    MsgTimeout = kwargs.pop('msg_timeout', None)

    m = re.match(r'(\d+)([kKmMgGtT]?)', kwargs.pop('max_file_size', "10M"))
    if m:
    ...
    if kwargs.get('certfile'):
        kwargs['certfile'] = os.path.abspath(kwargs['certfile'])
    else:
        kwargs['certfile'] = None
    if kwargs.get('keyfile'):
        kwargs['keyfile'] = os.path.abspath(kwargs['keyfile'])
    else:
        kwargs['keyfile'] = None
    ...
    bod = time.localtime()
    bod = (int(time.time()) - (bod.tm_hour * 3600) - (bod.tm_min * 60))
    if kwargs.get('service_start'):
        kwargs['service_start'] = time.strptime(kwargs.pop('service_start'), '%H:%M')
        kwargs['service_start'] = (bod + (kwargs['service_start'].tm_hour * 3600) +
                                          (kwargs['service_start'].tm_min * 60))
    if kwargs.get('service_stop'):
        kwargs['service_stop'] = time.strptime(kwargs.pop('service_stop'), '%H:%M')
        kwargs['service_stop'] = (bod + (kwargs['service_stop'].tm_hour * 3600) +
                                         (kwargs['service_stop'].tm_min * 60))
    if kwargs.get('service_end'):
        kwargs['service_end'] = time.strptime(kwargs.pop('service_end'), '%H:%M')
        kwargs['service_end'] = (bod + (kwargs['service_end'].tm_hour * 3600) +
                                        (kwargs['service_end'].tm_min * 60))
    del bod
    if (kwargs.get('service_start') or kwargs.get('service_stop') or
        kwargs.get('service_end')):
        if not kwargs.get('service_start'):
            kwargs['service_start'] = int(time.time())
        if not kwargs.get('service_stop') and not kwargs.get('service_end'):
            raise Exception('"service_stop" or "service_end" must also be given')
        if kwargs.get('service_stop'):
            if kwargs.get('service_start') >= kwargs.get('service_stop'):
                raise Exception('"service_start" must be before "service_stop"')
        if kwargs.get('service_end'):
            if kwargs.get('service_start') >= kwargs.get('service_end'):
                raise Exception('"service_start" must be before "service_end"')
            if (kwargs.get('service_stop') and
                kwargs.get('service_stop') >= kwargs.get('service_end')):
    ...
    if os.name == 'nt' and not kwargs.get('daemon'):
    ...
    if kwargs.get('node_init'):
        kwargs['node_init'] = os.path.abspath(os.path.expanduser(kwargs['node_init']))
        if not os.path.isfile(kwargs['node_init']):
            raise Exception('"init" file "%s" is not valid' % kwargs['node_init'])

    kwargs['node_globals'] = {}
    _dispy_q = multiprocessing.Queue()
    proc = multiprocessing.Process(target=_dispy_init_node,
                                   args=(kwargs.get('node_init'), _dispy_q, []))
    proc.start()
    proc.join()
    if proc.exitcode != 0:
        dispynode_logger.warning('"init" in %s failed with %s',
                                 kwargs.get('node_init'), proc.exitcode)
        exit(proc.exitcode)
    if kwargs.get('node_init'):
        kwargs['node_globals'] = _dispy_q.get()
        if not isinstance(kwargs.get('node_globals'), dict):
            dispynode_logger.warning('"init" in %s failed: %s', kwargs.get('node_init'),
                                     type(kwargs.get('node_globals')))
            exit(1)
    del _dispy_q, proc
    _dispy_node_globals = dict(kwargs.get('node_globals'))
    _dispy_node_init = kwargs.pop('node_init', None)
