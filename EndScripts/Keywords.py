...
from EndScripts.utils.deep_search import deep_search
...
def main(*args, queries: list=None, **kwargs):
	...
	text_data: list = deep_search(data, 'text', dict)

	if(not len(text_data)):
		return

	keywords = get_keywords([d['text'] for d in text_data])

	store_data({'keywords': keywords}, base_folder)
